...
class Login {
    ...
    public function __construct($_post_array) {
        ...
        elseif (!empty($_SESSION['user_name']) && ($_SESSION['user_logged_in'] == 1)) {
            ...
            if (isset($this->post_array["user_edit_submit_name"])) {
                
                $this->editUserName();
                
            } elseif (isset($this->post_array["user_edit_submit_email"])) {
                
                $this->editUserEmail();
                
            } elseif (isset($this->post_array["user_edit_submit_password"])) {
        ...
        } elseif (isset($this->post_array["login"])) {
        ...
        if (isset($this->post_array["request_password_reset"])) {
        ...
        } elseif (isset($this->post_array["submit_new_password"])) {
    ...
    private function loginWithPostData() {
        ...
        if (!empty($this->post_array['user_name']) && !empty($this->post_array['user_password'])) {
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                $this->user_name = $this->db_connection->real_escape_string($this->post_array['user_name']);            
                ...
                if ($checklogin->num_rows == 1) {
                    ...
                    if (password_verify($this->post_array['user_password'], $result_row->user_password_hash)) {
        ...
        } elseif (empty($this->post_array['user_password'])) {
    ...
    public function editUserName() {
        ...
        if (!empty($this->post_array['user_name']) && $this->post_array['user_name'] == $_SESSION["user_name"]) {
        ...
        elseif (!empty($this->post_array['user_name']) && preg_match("/^(?=.{2,64}$)[a-zA-Z][a-zA-Z0-9]*(?: [a-zA-Z0-9]+)*$/", $this->post_array['user_name'])) {
    ...
    public function editUserEmail() {
        ...
        if (!empty($this->post_array['user_email']) && $this->post_array['user_email'] == $_SESSION["user_email"]) {
            
            $this->errors[] = "Sorry, that email address is the same as your current one. Please choose another one.";
        
        } 
        // user mail cannot be empty and must be in email format
        elseif (!empty($this->post_array['user_email']) && filter_var($this->post_array['user_email'], FILTER_VALIDATE_EMAIL)) {
    ...
    public function editUserPassword() {
        ...
        if (empty($this->post_array['user_password_new']) || empty($this->post_array['user_password_repeat']) || empty($this->post_array['user_password_old'])) {
        ...
        } else if (!empty($this->post_array['user_password_old'])
            ...
            if (!$this->db_connection->connect_errno) {
                ...
                if ($check_for_right_password->num_rows == 1) {
                    ...
                    // using PHP 5.5's password_verify() function to check if the provided passwords fits to the hash of that user's password
                    if (password_verify($this->post_array['user_password_old'], $result_row->user_password_hash)) {
                        ...
                        $this->user_password_hash = password_hash($this->post_array['user_password_new'], PASSWORD_DEFAULT, array('cost' => $this->hash_cost_factor));                        
    ...
    public function editNewPassword() {
        ...
        if (!empty($this->post_array['user_name'])
            && !empty($this->post_array['user_password_reset_hash'])
            && !empty($this->post_array['user_password_new'])
            && !empty($this->post_array['user_password_repeat'])) {
                
            if ($this->post_array['user_password_new'] === $this->post_array['user_password_repeat']) {
         
                if ($this->post_array['user_password_new'] >= 6) {
                    ...
                    if (!$this->db_connection->connect_errno) {
                        ...
                        $this->user_name                = $this->db_connection->real_escape_string(htmlentities($this->post_array['user_name'], ENT_QUOTES));
                        $this->user_password_reset_hash = $this->db_connection->real_escape_string(htmlentities($this->post_array['user_password_reset_hash'], ENT_QUOTES));
